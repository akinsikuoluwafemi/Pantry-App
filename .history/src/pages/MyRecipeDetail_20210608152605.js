		...
		<>
			...
			<RecipeDetailSectionStyle className="container">
				...
				<RecipeDetailImage img={cateredRecipesDetail.image && cateredRecipesDetail.image} className="img-fluid">
					<span onClick={() => toggleFavourites(cateredRecipesDetail)}>
						{cateredRecipesDetail.favourite ? <FilledHeart /> : <NotFilledHeart />}
					</span>
				</RecipeDetailImage>
				...
				<RecipeStatsContainer>
					<StatsHeader>
						<span className="desc">Total Time</span>:{' '}
						{cateredRecipesDetail.totalTime && cateredRecipesDetail.totalTime === '0'
							? ConvertCookedTime(25)
							: ConvertCookedTime(cateredRecipesDetail.totalTime)}{' '}
					</StatsHeader>
					{/* 
						<StatsHeader>
							<span className="desc">Calories</span>: {calories && calories.toFixed(2)}{' '}
					...
					<StatsHeader>
						<span className="desc">Servings</span>:{' '}
						{cateredRecipesDetail.servings && cateredRecipesDetail.servings} Servings{' '}
					</StatsHeader>
						...
						<NutrientsChipRow>
							<OuterWrapperChip>
								<div className="quantity">
									{getToFixedValue(cateredRecipesDetail.totalNutrients.CHOCDF.quantity)}
								</div>
								<div>{cateredRecipesDetail.totalNutrients.CHOCDF.label}</div>
							</OuterWrapperChip>
							<OuterWrapperChip>
								<div className="quantity">
									{getToFixedValue(cateredRecipesDetail.totalNutrients.PROCNT.quantity)}
								</div>
							...
							<OuterWrapperChip>
								<div className="quantity">
									{getToFixedValue(cateredRecipesDetail.totalNutrients.SUGAR.quantity)}
								</div>
								<div>{cateredRecipesDetail.totalNutrients.SUGAR.label}</div>
							</OuterWrapperChip>
							<OuterWrapperChip>
								<div className="quantity">
									{getToFixedValue(cateredRecipesDetail.totalNutrients.WATER.quantity)}
								</div>
