     ...
     return (
			<div>
				...
				<RecipeStatsContainer>
					<StatsHeader>
						<span className="desc">Total Time</span>: <Tag>{totalTime === 0 ? 25 : totalTime} mins</Tag>{' '}
					</StatsHeader>

					<StatsHeader>
						<span className="desc">DietLabels</span>:{' '}
						{dietLabels.map((dietLabel) => (
							<Tag>{dietLabel}</Tag>
						))}{' '}
					</StatsHeader>
					<StatsHeader>
						<span className="desc">Meal Type</span>:{' '}
						{mealType.map((meal) => (
							<Tag>{meal}</Tag>
						))}{' '}
					</StatsHeader>
					...
					<StatsHeader>
						<span className="desc">Calories</span>: <Tag>{calories.toFixed(2)}</Tag>{' '}
					</StatsHeader>
					<StatsHeader>
						<span className="desc">Servings</span>: <Tag>{servings} Servings</Tag>{' '}
					</StatsHeader>

					<StatsHeader>
						<span className="desc"></span>HealthLabels:{' '}
