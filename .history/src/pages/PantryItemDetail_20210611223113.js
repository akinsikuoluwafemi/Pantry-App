...
import {
	...
	PantryDetailImage,
	PantryExpiryTextTitle,
} from '../styles/PantryDetailStyles';
...
function PantryItemDetail(props) {
	...
	const { name, notes, weight, quantity, expiry, imageSrc, unit } = pantryDetail;
	...
	return (
		<>
			...
			>
				...
				<Space size="middle" style={{padding: '0 0.5rem'}}>
				...
				<PantryDetailValuesWrapper>
					...
					<PantryDetailTimeWrapper>
						<PantryExpiryTextTitle>
							Best Before: <span style={{fontWeight: 'normal'}}>{moment(expiry).format('DD-MM-YYYY')}</span>
						</PantryExpiryTextTitle>
{/* 
						<PantryExpiryTextTitle>
							Time Left:{' '}
							<span style={{fontWeight: 'normal'}}>{getExpiryTimeDifference(moment(expiry), moment(moment(createdAt).format('DD-MM-YYYY')))}</span>
						</PantryExpiryTextTitle> */}
					</PantryDetailTimeWrapper>
				</PantryDetailValuesWrapper>
