				...
				<RecipeDetailSectionStyle className="container">
					...
					<RecipeStatsContainer>
						<StatsHeader>
							...
							{totalTime && totalTime === '0' ? ConvertCookedTime(25) : ConvertCookedTime(totalTime)}</Tag>{' '}
						</StatsHeader>

						<StatsHeader>
							<span className="desc">Calories</span>: {calories && calories.toFixed(2)}</Tag>{' '}
						</StatsHeader>
						<StatsHeader>
							<span className="desc">Servings</span>: {servings && servings} Servings</Tag>{' '}
						</StatsHeader>
