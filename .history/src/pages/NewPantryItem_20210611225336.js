...
import { createPantryItem, setItemName, addToCategory } from '../redux/actions/pantryActions';
...
import { PlusOutlined } from '@ant-design/icons';
 ...
 function NewPantryItem(props) {
	 ...
	 const { createPantryItem, setItemName, itemName, showFetchRecipeButton, categoryArr, addToCategory } = props;
	...
	const [pic, setPic] = useState('');
	 ...
	 const addItem = () => {
		 ...
		 if(category.length > 2){
				setState({
					...state,
					// options: [...options, category.replace(/[^0-9a-z]/gi, '')],
					options: [...options, category.toLowerCase().replace(/[^0-9a-z]/gi, '')],
	...
	const handleNameChange = (e) => {
	 ...
	 const onCategorySelectChange = (value) => {
			...
			console.log(value);
		 		setItemName(name);
				addToCategory(value.toLowerCase());
	...
	const handleSubmit = (e) => {
		...
		if (state.categoryValue.length !== 0 && name.length !== 0) {
			...
			setTimeout(() => {
				...
				const { options, categoryValue } = state;
				setState({
						...state,
						options: [...options, categoryValue],
						category: '',
					});
	 ...
	 return (
			<>
				...
				<NewPantrySection
					...
					className="container"
				>
					<HeaderTitle large style={{padding: '-1rem'}}>Add Pantry Item</HeaderTitle>
					{/* <Form> */}
 ...
 const mapStateToProps = ({ pantry: { itemName, categoryArr }, cateredRecipe }) => {
