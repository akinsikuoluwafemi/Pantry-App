...
import MedRecipesList from '../components/MedRecipesList';
 ...
 function MyRecipe(props) {
	 ...
	 const previousItemNameRef = useRef(itemName)

	 
	 
	useEffect(() => {
		if (previousItemNameRef.current !== itemName) {
			fetchRecipes(previousItemNameRef,
			...
			previousItemNameRef.current = itemName;
		}
	}, [ itemName, fetchAsianRecipes, fetchMedRecipes, fetchRecipes]);
	 
	 console.log(itemName, previousItemNameRef.current)
