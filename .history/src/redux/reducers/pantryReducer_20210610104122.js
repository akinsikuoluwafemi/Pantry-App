...
// const removedDuplicatesArr = removeDuplicates(categoryArr);
    ...
    switch(action.type){
        case pantryTypes.CREATE_PANTRY_ITEM:
			return {
				...
				pantryArrCopy: [action.payload, ...state.pantryArr],
			};
		case pantryTypes.REMOVE_PANTRY_ITEM:
			return {
				...state,
				pantryArr: state.pantryArr.filter((item) => item.id !== action.payload.id),
				pantryArrCopy: state.pantryArr.filter(item => item.id !== action.payload.id)
			};
		case pantryTypes.EDIT_PANTRY_ITEM:
			return {
				...state,
				pantryArr: state.pantryArr.map(item => item.id === action.payload.id ? action.payload  : item),
				pantryArrCopy: state.pantryArr.map(item => item.id === action.payload.id ? action.payload  : item)
			}
		...
		case pantryTypes.ADD_TO_CATEGORY:
			return {...state, categoryArr: removeDuplicates([...state.categoryArr , action.payload]}
