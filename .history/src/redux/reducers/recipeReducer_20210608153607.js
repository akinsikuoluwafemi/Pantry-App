...
const recipeReducer = (state=INITIAL_STATE, action) => {
    switch (action.type) {
		...
		case pantryTypes.ADD_TO_FAVOURITE:
			return {
				...state,
				favRecipesArr: [{ ...action.payload, favourite: !action.payload.favourite }, ...state.favRecipesArr],
				// [action.payload, ...state.pantryArrCopy]
			};
