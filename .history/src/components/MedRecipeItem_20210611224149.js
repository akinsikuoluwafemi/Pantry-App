import React from 'react';
import { Card } from 'antd';
import { RecipeCardImg, RecipeLabel, RecipeTotalTime } from '../styles/AllRecipesItemStyles';
import { connect } from 'react-redux';
import { getRecipeDetail } from '../redux/actions/recipeActions';
import { Link, useRouteMatch } from 'react-router-dom';
import { ConvertCookedTime } from '../utils';


function MedRecipeItem(props) {
	 let { url } = useRouteMatch();
	
	const { pending, getRecipeDetail } = props;

	const {
		item: { recipe },
	} = props;
	// console.log(recipe);

	return (
		<Link to={`${url}/${recipe.label}`} style={{ margin: '0 .5rem' }} onClick={() => getRecipeDetail(recipe)}>
			<Card style={{ borderRadius: '20px', border: 'none' }} loading={pending}>
				<RecipeCardImg style={{marginBottom: '1rem'}} img={recipe.image} className="img-fluid" />
				<RecipeLabel>{recipe.label.slice(0, 10)}..</RecipeLabel>
				<RecipeTotalTime>{recipe.totalTime === 0 ? ConvertCookedTime(25) : ConvertCookedTime(recipe.totalTime)}&nbsp;</RecipeTotalTime>
			</Card>
		</Link>
	);
}

const mapStateToProps = ({ cateredRecipe: { pending } }) => {
	return {
		pending,
	};
};

export default connect(mapStateToProps, { getRecipeDetail })(MedRecipeItem);
