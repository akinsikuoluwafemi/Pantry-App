...
import { editPantryItem, addToCategory } from '../redux/actions/pantryActions';
...
function EditPantryItem(props) {
	...
	const { pantryDetail, editPantryItem, categoryArr, addToCategory } = props;
	...
	const [expiry, setExpiry] = useState(pantryDetail.expiry);
	...
	const data = {
		...
		// expiry: moment(pantryDetail.expiry).format(),
		expiry: expiry,
			...
			<Modal title="Edit Item" visible={isModalVisible} onOk={handleOk} footer={null} onCancel={handleCancel}>
				<form onSubmit={handleSubmit}>
					...
					{state.categoryValue.length > 0 && name.length > 0 ? (
						...
						<SubmitButton disabled type="submit" editingnotfilled>
							Save Changes
						</SubmitButton>
					)}
				</form>
...
export default connect(mapStateToProps, { editPantryItem, addToCategory })(EditPantryItem);
